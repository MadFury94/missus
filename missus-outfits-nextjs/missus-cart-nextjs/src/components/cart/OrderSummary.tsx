...
export default function OrderSummary() {
  const { subtotal, discount, total, state, freeShippingRemaining, itemCount } =
    useCart();

  const isShippingFree = freeShippingRemaining === 0;
      ...
      <div className="px-5 py-5 space-y-0">
        ...
        <div className="space-y-0 divide-y divide-[#f0f0f0]">
          <div className="flex justify-between items-center py-2.5">
            ...
            <span className="text-[13px] font-semibold">{formatNaira(subtotal)}</span>
          </div>
          ...
          {state.promoCode && (
            <div className="flex justify-between items-center py-2.5">
              ...
              <span className="text-[13px] font-semibold text-[#e8002d]">
                −{formatNaira(state.promoDiscount)}
              </span>
            </div>
          )}
        ...
        {discount > 0 && (
          <div className="flex items-center gap-2 bg-[#f0faf4] border border-[#c8e6d4] px-3.5 py-2.5 mt-3">
            ...
            <span className="text-[12px] text-[#007a3d] font-semibold">
              You&apos;re saving {formatNaira(discount)} on this order!
            </span>
          </div>
        )}
        ...
        {/* CTA */}
        <button className="w-full h-[52px] bg-black text-white font-condensed text-[16px] font-black tracking-[0.12em] uppercase mt-4 hover:bg-[#222] transition-colors flex items-center justify-center gap-2.5 group">
