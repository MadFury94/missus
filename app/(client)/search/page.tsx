...
export default function SearchPage() {
    ...
    // Run search whenever query or sort changes
    useEffect(() => {
        if (!query.trim()) {
            setProducts([]);
            return;
        ...
        fetch(`/api/search?q=${encodeURIComponent(query)}`, { signal: controller.signal })
            ...
            .then((data) => {
                ...
                // Client-side sort (search API returns by relevance)
                if (sort === "date") {
                    // WooCommerce Store API returns date_on_sale_from or we fall back to id (higher = newer)
                    results = [...results].sort((a, b) => b.id - a.id);
                } else if (sort === "price-asc") {
                    results = [...results].sort(
                        (a, b) => parseInt(a.prices.price) - parseInt(b.prices.price)
                    );
                } else if (sort === "price-desc") {
                    results = [...results].sort(
                        (a, b) => parseInt(b.prices.price) - parseInt(a.prices.price)
                    );
                }
                setProducts(results);
            })
                ...
                <div style={{ maxWidth: "680px", margin: "0 auto" }}>
                    <form onSubmit={handleSubmit} style={{ display: "flex", border: "2px solid #fff", height: "52px" }}>
                        <div style={{ position: "relative", flex: 1 }}>
                            <Search style={{ position: "absolute", left: "14px", top: "50%", transform: "translateY(-50%)", width: "18px", height: "18px", color: "rgba(255,255,255,.5)", pointerEvents: "none" }} />
                            <input
                                ref={inputRef}
                                value={inputVal}
                            ...
                            {inputVal && (
                                <button
                                    type="button"
                                    onClick={clearSearch}
                                    style={{ position: "absolute", right: "12px", top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", color: "rgba(255,255,255,.5)", padding: 0, display: "flex" }}
                                >
                        ...
                        <button
                            type="submit"
                            style={{ background: "#fff", color: "#000", border: "none", paddingInline: "24px", fontFamily: "'Barlow Condensed', sans-serif", fontSize: "13px", fontWeight: 700, letterSpacing: ".1em", textTransform: "uppercase", cursor: "pointer", whiteSpace: "nowrap", flexShrink: 0 }}
                        >
                            Search
                        </button>
                    </form>
                    ...
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "20px", paddingBottom: "14px", borderBottom: "1px solid #e8e8e8", flexWrap: "wrap", gap: "12px" }}>
                        ...
                        {products.length > 0 && (
                            <select
                                ...
                                style={{ fontFamily: "'Barlow', sans-serif", fontSize: "12px", border: "1px solid #e0e0e0", padding: "8px 28px 8px 10px", background: "#fff", cursor: "pointer", outline: "none" }}
                            >
                                {SORT_OPTIONS.map((o) => (
