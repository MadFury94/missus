...
function validContent(value: unknown): value is HomepageContent {
    ...
    return strings(content.announcement, content.newsletter?.heading, content.newsletter?.sub)
        ...
        ) && optionalString(slide.mobileSrc))
        && Array.isArray(content.styleRadar) && content.styleRadar.every(card => card && strings(card.title, card.href, card.img))
        && content.categories && typeof content.categories === "object"
        && content.categories.feature && strings(content.categories.feature.label, content.categories.feature.href, content.categories.feature.img)
        && Array.isArray(content.categories.grid) && content.categories.grid.every(cat => cat && strings(cat.label, cat.href, cat.img));
}
